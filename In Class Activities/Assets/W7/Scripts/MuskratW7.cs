...
public class MuskratW7 : MonoBehaviour
{
    ...
    private void MoveOrbitMode()
    {
        ...
        //TransformDirection switched the rotation of the muskrat from globally relative to locally relative
        Vector3 modUp = transform.TransformDirection(Vector3.up);
        transform.RotateAround(_sphereTransform.position, modUp, leftright * _rotationSpeed * Time.deltaTime);
    ...
    private void MoveNormal()
    {
        ...
        else if(Mathf.Abs(_rigidbody.linearVelocity.y) <= 0.1f)
        {
            _animator.SetBool("flying", false);
        }
        ...
        else if(Mathf.Abs(_rigidbody.linearVelocity.x) == 0.0f)
        {
